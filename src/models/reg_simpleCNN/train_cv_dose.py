"""
LOSO cross-validation for SimpleCNNDose (dose-injection experiments).

Same protocol and hyperparameters as train_cv.py (apr19 winning stack:
Adam lr=5e-4, bs=64, Huber beta=10, wd=1e-4, dropout=0.3, patience=40 on
val_r2, EMA decay=0.999 per batch, no mixup, no LR schedule, 64/128/256).
Only difference: the model receives per-sample dose_mg alongside EEG.

Mirrors main's train_cv.py features: per-fold DMT evolution line plots and
--subjects restricting the training pool as well as the CV pool.

Usage (run from project root):
    python -m src.models.reg_simpleCNN.train_cv_dose \
        --dose_mode multiplicative --run_name dose_multiplicative
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="LOSO CV for dose-aware SimpleCNN on DMT plasma regression"
    )
    p.add_argument("--dose_mode", type=str, required=True,
                   choices=["multiplicative", "multiplicative_residual", "concat"])
    ...
    p.add_argument("--subjects", nargs="+", default=None,
                   help="Restrict both CV folds AND the training pool to these "
                        "subject IDs (default: ALL_SUBJECTS)")
    p.add_argument("--experiment_name", type=str,
                   default="SimpleCNN_DMT_regression_CV_dose")
...
def run_fold(args, val_subject, fold_idx, n_folds, device,
    ...
    """Train one dose-aware LOSO fold. Same return contract as train_cv.run_fold.

    If parent_run_id + mlf_client are provided, per-epoch training curves and
    per-fold plots (scatter + DMT evolution) are mirrored on the parent run.
    """
    ...
    print(f"  FOLD {fold_idx}/{n_folds}  |  val_subject = {val_subject}  "
          f"|  dose_mode = {args.dose_mode}")
    print("=" * 70, flush=True)
    print(f"Train subjects ({len(train_subjects)}): {train_subjects}")
    print(f"Val subject:                 {val_subject}")

    train_ds = EEGDoseDataset(subjects=train_subjects)
    val_ds = EEGDoseDataset(subjects=[val_subject])
    ...
    train_doses = sorted(set(float(d) for d in train_ds.doses.tolist()))
    val_doses = sorted(set(float(d) for d in val_ds.doses.tolist()))
    print(f"Train doses (mg): {train_doses}")
    print(f"Val dose (mg):    {val_doses}")
    ...
    model = SimpleCNNDose(in_channels=train_ds.n_channels,
                          dropout=args.dropout,
                          dose_mode=args.dose_mode).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model parameters: {n_params:,}  (dose_mode={args.dose_mode})")

    ema_model = SimpleCNNDose(in_channels=train_ds.n_channels,
                              dropout=args.dropout,
                              dose_mode=args.dose_mode).to(device)
    ...
    mlflow.log_params({
        ...
        "val_dose_mg": val_doses[0] if len(val_doses) == 1 else ",".join(map(str, val_doses)),
    })
    ...
    plot_predicted_vs_actual(
        y_true, y_pred,
        title=f"Fold {fold_idx}/{n_folds} (val={val_subject}, {args.dose_mode})",
    ...
    plot_dmt_evolution(
        ...
        title=(f"DMT evolution — fold {fold_idx}/{n_folds} "
               f"(val={val_subject}, {args.dose_mode})\n"
...
def main():
    ...
    print("=" * 70)
    print("  SimpleCNNDose DMT Plasma Regression  |  LOSO Cross-Validation")
    print("=" * 70)
    print(f"Device:        {device}")
    print(f"Subjects ({n_folds}): {cv_subjects}")
    print(f"Dose mode:     {args.dose_mode}")
    print(f"Hyperparams:   lr={args.lr}  bs={args.batch_size}  "
          f"dropout={args.dropout}  weight_decay={args.weight_decay}  "
          f"loss={args.loss}(beta={args.huber_beta})  ema={args.ema_decay}")
    ...
    run_name = args.run_name or f"cv_dose_{args.dose_mode}"
    ...
    with mlflow.start_run(run_name=run_name) as parent_run:
        ...
        for i, subj in enumerate(cv_subjects, start=1):
            ...
            for k, v in [
                ...
                mlf_client.log_metric(parent_run_id, f"fold_{subj}_{k}", v)
        ...
        plot_predicted_vs_actual(
            all_true, all_pred,
            title=f"LOSO CV pooled ({n_folds} folds, dose_mode={args.dose_mode})",
            metrics=pooled,
        ...
        print(
            f"  RESULT dose_mode={args.dose_mode} "
            f"cv_mean_val_r2={agg['best_val_r2'][0]:+.4f} "
