"""
Training script for SimpleCNN EEG regression.

Predicts plasma DMT concentration (ng/mL) from 3-second EEG trials
using a 1-D CNN with SE blocks.

Usage (run from project root):
    python -m src.models.reg_simpleCNN.train --val_subjects S12 \
...
def parse_args():
    p = argparse.ArgumentParser(description="Train SimpleCNN on EEG DMT plasma regression")
    p.add_argument("--val_subjects", nargs="+", default=["S12"],
...
def compute_regression_metrics(y_true, y_pred):
    """Compute MAE, RMSE, MSE, R² for regression."""
...
def plot_latent_space(model, train_loader, val_loader, val_subjects, device):
    """t-SNE of 128-dim pre-head features, colored by plasma concentration."""
    model.eval()
    ...
    fig.suptitle("Latent space (128-dim CNN features -> t-SNE)", fontsize=13)
    plt.tight_layout()
...
def evaluate(model, loader, criterion, device):
    ...
    all_labels = np.concatenate(all_labels)
    metrics = compute_regression_metrics(all_labels, all_preds)
    return total_loss / n_samples, metrics, all_labels, all_preds
